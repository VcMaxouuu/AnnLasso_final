...
class LinearModel():
    def __init__(self, path_type=1, lambda_qut=None, device=None):
    ...
    def fit(self, X, y, verbose=False):
        ...
        # Training
        for i in range(-1, 6):
            nu = 1 if self.path_type == 0 else [1, 0.8, 0.6, 0.4, 0.2, 0.1, 0.01][i + 1]
            lambi = self.lambda_qut * (np.exp(i) / (1 + np.exp(i)) if i < 5 else 1)
            rel_err = 1e-9 if i == 5 else 1e-5

            if verbose:
                print(f"Lambda = {lambi.item():.4f} -- Nu = {nu}" if self.path_type == 1 else f"Lambda = {lambi.item():.4f}")
            
            self.train_model(nu, X, y, lambi, 0.1, rel_err, verbose)
    ...
    def important_features(self):
        imp_feat = torch.nonzero(self.theta.weight.squeeze(), as_tuple=False).squeeze()
        if imp_feat.ndim == 0: imp_feat = [imp_feat.item()]
        else: imp_feat = imp_feat.tolist()
        return len(imp_feat), imp_feat
    ...
    def train_model(self, nu, X, y, lambda_, init_lr, rel_err, verbose):
        ...
        while epoch < max_epochs:
            ...
            loss, bare_loss, train_loss = loss_fn(y_pred, y, self.theta)
            loss = loss.detach()
            train_loss = train_loss.detach()

            if verbose and epoch % 20 ==0:
                print(f"\tEpoch: {epoch} | Loss: {loss.item():.5f} | MSE on train set: {train_loss.item():.5f} | Non zeros parameters: {self.important_features()} | learning rate : {optimizer.get_lr():.6f}")
