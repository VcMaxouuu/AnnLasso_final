...
if __name__ == '__main__':
    for n in [160, 190]:
        for scaler in [0.8, 1, 1.2]:  
            X = pd.read_csv(f"data/linear/n{n}/X-n{n}-p300.csv")
            X_tensor = utils.X_to_tensor(X)
            lambda_qut = scaler*utils.lambda_qut_regression(X_tensor, utils.Custom_act_fun())
            ...
            for s in s_values:
                ...
                if not any(cont.values()):
                    ...
                    df.to_csv(f"data/linear/n{n}/PESR-{scaler}lambda.csv", index=False)
                    continue
